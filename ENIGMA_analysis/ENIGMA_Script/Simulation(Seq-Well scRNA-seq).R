...
res_alg_all4 <- cell_deconvolve(X=log2(Bulk+1),
                                ...
                                beta=0.5,tao_k=0.1,max.iter=1000,verbose=TRUE,Normalize=TRUE,Norm.method = "frac")
save(res_alg_all3,res_alg_all4,file = "./ENIGMA_l2_seqwell.Rdata")
...
res_alg_trace3<-cell_deconvolve_trace(O=log2(as.matrix(Bulk)+1),
                                      ...
                                      R=log2(profile+1),
                                      epsilon=0.001,
                                      alpha=alpha,beta=1,solver="proximalpoint",gamma=1,
...
res_alg_trace4<-cell_deconvolve_trace(O=sqrt(as.matrix(Bulk)),
                                      ...
                                      alpha=alpha,beta=1,solver="proximalpoint",gamma=1,
                                      verbose=F,max.iter = 500,Normalize=T,pos=T,Norm.method = "frac")
save(res_alg_trace3,res_alg_trace4,file = "./ENIGMA_trace_seqwell_posT.Rdata")

#### 2.Caculate Correlation
## 2.1 Correlation per gene
# 2.1.1 TCA
load("./TCA_seqwell.Rdata")
tca_res2=Z_hat2
tca_res3=Z_hat3
...
# 2.1.2 bMIND
load("./bMIND_seqwell_rmlog2.Rdata")
...
# 2.1.4 ENIGMA-trace
load("./ENIGMA_trace_seqwell_posT.Rdata")
...
save(res_enigma_s2,res_enigma_g2,file="./ENIGMA_rmlog2_RMSE.Rdata")
...
data=rbind(data,dat)
load(file="./cor_ENIGAMA_trace_gene.Rdata")
data=rbind(data,dat)
...
png("./results_figs/fig2_cor_gene_best2.png",res=300,height=1000,width=2300)
...
data=rbind(data,dat)
load(file="./cor_ENIGAMA_trace_sample.Rdata")
data=rbind(data,dat)
...
p=ggplot(data2, aes(x=celltype, y=performance, fill=Method)) +
  geom_boxplot()+theme_minimal()+labs(y="RMSE per gene")+facet_grid(~celltype, scales = "free_x") +
...
png("./results_figs/fig2_rmse_gene_best2.png",res=300,height=1000,width=2300)
...
png("./results_figs/fig2_rmse_sample_best2.png",res=300,height=1000,width=2300)
p+scale_fill_npg(alpha=0.9)
dev.off()
